...
import torch
from utils import is_png_file, load_img, Augment_RGB_torch,load_tiff,is_tiff_file,is_png_file
...
##################################################################################################
class DataLoaderVal_denoising_sidd_tiff(Dataset):
    def __init__(self, rgb_dir, target_transform=None):
        super(DataLoaderVal_denoising_sidd_tiff, self).__init__()
        self.target_transform = target_transform
        noisy_files = sorted(os.listdir(os.path.join(rgb_dir)))
        self.noisy_filenames = [os.path.join(rgb_dir, x) for x in noisy_files if is_tiff_file(x)]
    ...
    def __getitem__(self, index):
        ...
        # tt = load_tiff(self.noisy_filenames[tar_index])

        noisy = torch.from_numpy(np.float32(load_tiff(self.noisy_filenames[tar_index])))
        noisy_filename = os.path.split(self.noisy_filenames[tar_index])[-1]
        noisy = noisy.permute(2, 0, 1)
        return noisy, noisy_filename

class DataLoaderVal_denoising_sidd_png(Dataset):
    def __init__(self, rgb_dir, target_transform=None):
        super(DataLoaderVal_denoising_sidd_png, self).__init__()
        self.target_transform = target_transform
        noisy_files = sorted(os.listdir(os.path.join(rgb_dir)))
        self.noisy_filenames = [os.path.join(rgb_dir, x) for x in noisy_files if is_png_fil(x)]
        self.tar_size = len(self.noisy_filenames)
